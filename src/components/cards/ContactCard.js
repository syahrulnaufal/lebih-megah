...
export default function ContactCard({ role, name, phone, accentColor = 'var(--color-primary-light)' }) {
  const cleanPhone = phone.replace(/[^0-9]/g, '');
  const waLink = `https://wa.me/${cleanPhone}`;

  return (
    <div 
      className="glass-panel p-8 rounded-[2.5rem] bg-navy-dark/20 text-center relative overflow-hidden group hover:border-[var(--color-primary-light)]/30 transition-all duration-500"
      style={{
    ...
    >
      ...
      <a
        ...
        onMouseLeave={(e) => {
          e.currentTarget.style.borderColor = `color-mix(in srgb, ${accentColor} 25%, transparent)`;
