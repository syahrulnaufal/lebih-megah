...
export default function PillarCard({ icon: Icon, title, description, className = '', style = {} }) {
  return (
    <div 
      className={`relative p-8 md:p-10 rounded-[2.5rem] glass-navy border border-[var(--color-primary-light)]/10 hover:border-[var(--color-primary-light)]/30 transition-all duration-500 group overflow-hidden card-hover ${className}`}
      style={style}
    >
      ...
      <div className="flex items-start gap-5 relative z-10 text-left">
        {Icon && (
          <Icon
            className="w-10 h-10 text-[var(--color-primary-light)] shrink-0 group-hover:scale-110 transition-transform duration-300"
            aria-hidden="true"
          />
        )}
        <div>
